...
def test_vote_on_post_non_exists(authorized_client, test_posts, test_vote):
    res = authorized_client.post("/vote/", json = {"post_id": 9999999, "dir": 1})
    
    assert res.status_code == 404

def test_unauthentucated_try_to_vote_on_post(client, test_posts, test_vote):
    res = client.post("/vote/", json = {"post_id": 9999999, "dir": 1})
